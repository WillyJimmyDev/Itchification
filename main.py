...
def main():
    ...
    class MainWindow(QMainWindow, QtCore.QObject):
        global twitch
        list_view = QListView(editTriggers=QAbstractItemView.NoEditTriggers)

        def __init__(self):

            super().__init__()
            stylesheet_path = os.path.join(
                os.path.dirname(__file__), "styles/style.qss"
            )
            ...
            followed_list.sort(
                key=lambda l: (-l["live"], l["display_name"].casefold())
            )  # sort by live status
            ...
            )  # .activated is sent when doubleclicked or enter key pressed
